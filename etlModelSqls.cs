...
namespace Datamech.mssql
{
    public class Sqls
    {
        ...
        private List<string> PrimaryKeyFields
        {
            get
            {
                return this.ParentModel.EtlModel.SourceKeyFilelds;
            }
        }
        ...
        public string GetCreateTableSql()
        {
            ...
            foreach (var kv in this.source.Dbs.Rows)
            {
                ...
                string nullable = string.Empty; // "null" vs "not null"
                string fieldName = this.TouchMsSqlName(colName);

                if (ParentModel.EtlModel.SourceKeyFilelds.Contains<string>(fieldName))
                {
                    nullable = "not null";
                }
                else
                ...
                string item = string.Concat(fieldName, ' ', colType, ' ', nullable); // null vs not null
                sqlRows.Add(item);
            }
            ...
            {
                sb.AppendLine(string.Concat(string.Join(",\n", sqlRows), ','));
                string pkname = this.TouchMsSqlName("PK_" + this.target.TableName);
                string constraint = $"CONSTRAINT {pkname} PRIMARY KEY ";
                string pk = '(' + string.Join(',', this.PrimaryKeyFields) + ')';
                sb.AppendLine(string.Concat(constraint, pk));
            }
            else
            {
