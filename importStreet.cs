...
public class PgImportStreet : PgImportBase
{
    public PgImportStreet(ObjectInfo objectInfo) : base(objectInfo)
    {
    }
        ...
        if (!clearDestTableInAdvance || (clearDestTableInAdvance && this.clearDestTable()))
        {
            ...
            using (var dbfDataReader = new DbfDataReader.DbfDataReader(this.objectInfo.SourceFilePath, this.dbfOptions))
            {
                ...
                {   // Street
                    ...
                    var bcmd = new NpgsqlBatchCommand("INSERT INTO public.street(name, socr, code, index, gninmb, uno, ocatd) VALUES (@name, @socr, @code, @index, @gninmb, @uno, @ocatd);");
                ...
                Log.Information("{table} import duration: {duration}, imported {recs}", this.objectInfo.DestinationSchemaName, duration, RecordCount);
            }
        }
    }
}
