...
namespace Shared.connections
{
    public class MsSqlConnection
    {
        ...
        {
            Log.Information("Проверка подключения по строке: {connString}", this.ConnectionString);
            try
            {
        ...
        public bool CheckMsSqlSelectSql(string selectSql)
        {
            Log.Information("Тестирование корректности SelectSQL");
            Log.Debug(selectSql);
            ...
            using (SqlConnection conn = new SqlConnection(this.ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand(selectSql, conn))
                {
                    ...
                    {
                        Log.Debug("Ошибка чтения данных \n {errmes}", ex.Message);
                        return false;
                    }
                }
            }
