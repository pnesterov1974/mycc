...
namespace Datamech.mssql
{
    public class RunModel
    {
        ...
        private bool etlModelIsValue = false;
        ...
        public RunModel(IEtlModel etlModel)
        {
            try
            {
                ...
                this.etlModelIsValue = vm.Validate();
                
                this.source = new Source(parentModel: this);
                this.target = new Target(parentModel: this);

                if (this.etlModelIsValue)
                {
                    bool sameDbInstace = Connections.ConnectToTheSameMsSqlInstance(this.source.ConnectionString, this.target.ConnectionString);
        ...
        public bool Run()
        {
            ...
            {
                if (!this.Source.ConnectionOk)
                {
                    Log.Information("Отсутствует подключение к источнику данных Source.");
                }
                if (!this.Target.ConnectionOk)
                {
                    Log.Information("Отсутствует подключение к источнику данных Target.");
                }
                return false;
            }
            else
