    ...
    public class ValidateModel
    {
        ...
        public bool Validate(bool useKeyFieldsWhenValidating = false)
        {
            ...
            if (modelHasSourceDbName)
            {
                ...
                try
                {
                    string conn = ConnectionStrings.GetConnectionStringByName(this.EtlModel.SourceDbName);
            ...
            if (modelHasTargetDbName)
            {
                ...
                if (modelHasTargetDbName)
                {
                    ...
                    try
                    {
                        string conn = ConnectionStrings.GetConnectionStringByName(this.EtlModel.TargetDbName);
                        modelHasTargetDbConnectionString = (conn.Length > 0);
                        ...
                        switch (this.targetDb)
                        {
                            ...
                            case TargetDb.pgsql:
                                var pgConn = new MsSqlConnection(conn);
                                modelTargetConnectionIsValid = pgConn.TryMsSqlDbConnection();
                                break;
                        }
                    }
                    catch (Exception ex)
